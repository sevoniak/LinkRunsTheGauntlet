...
*/
	...
	spriteBatch = new SpriteBatch(m_D3D->GetDeviceContext());

	m_font = new SpriteFont(m_D3D->GetDevice(), L"gameFont.spritefont" );
	return true;
}
...
bool GraphicsClass::Render()
{
	...
	if (sprites)
	{
		spriteBatch->Begin(SpriteSortMode_Deferred, m_D3D->getBlendState(), nullptr, m_D3D->getStencilState(), m_D3D->getRasterState());
		for (int i = 0; i < sprites->size(); i++)
		{
			if (sprites->elementAt(i))
				sprites->elementAt(i)->Draw(spriteBatch, t);
		}
		...
		if (output)
		{
			...
			{
				wstring text = output->elementAt(i)->getText();
				const WCHAR* textOut = text.c_str();
				m_font->DrawString(spriteBatch, textOut, output->elementAt(i)->getPos(), Colors::Blue);
			}
		}
		spriteBatch->End();
	}
...
void GraphicsClass::setSprite(ArrayList<Sprite>* sp) 
{ 
	sprites = sp; 
}

void GraphicsClass::setFont(ArrayList<FontItem>* fonts)
{
	output = fonts;
}

bool GraphicsClass::resetGameModels(ArrayList<GameModel>* gameModels)
{
	m_GameWorldModels = gameModels;
	bool result = true;
	...
	if(m_GameWorldModels && !m_GameWorldModels->isEmpty())
	{
		for(int i=0; i< m_GameWorldModels->size(); i++)
		{
	        ...
	        if(!result)
	        {
		    //   MessageBox(hwnd, L"Could not reset the game model object.", L"Error", MB_OK);
		       return false;
	        }
			if(gameModel->isTextureVertexModel()){
				result = gameModel->initializeTextures(m_D3D->GetDevice());
	            if(!result)
	            {
		          //  MessageBox(hwnd, L"Could not reset the game model textures.", L"Error", MB_OK);
		            return false;
	            }
			}
		}
